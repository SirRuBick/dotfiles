...
function M.toggle_ui_notifications()
  ...
  vim.notify(string.format("ui notifications %s", bool2str(vim.g.ui_notifications_enabled)))
end
...
function M.toggle_cmp()
  ...
  vim.notify(ok and string.format("completion %s", bool2str(vim.g.cmp_enabled)) or "completion not available")
end
...
function M.toggle_buffer_autoformat()
  ...
  vim.notify(string.format("Buffer autoformatting %s", bool2str(vim.b.autoformat_enabled)))
end
...
function M.toggle_buffer_semantic_tokens(bufnr)
  ...
  for _, client in ipairs(vim.lsp.get_active_clients()) do
    if client.server_capabilities.semanticTokensProvider then
      ...
      vim.notify(string.format("Buffer lsp semantic highlighting %s", bool2str(vim.b.semantic_tokens_enabled)))
    end
  end
end
